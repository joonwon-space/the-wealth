...
| 예수금 조회 | 국내+해외 합산 예수금 조회 (Redis 캐시 30초); total_cash = KRW 예수금 + USD cash(CTRP6504R 환산); total_evaluation = 주식 평가금액 (현금 제외, 이중합산 방지); foreign_cash(USD 원액) + usd_krw_rate 노출 |
...
| SSE 조건 체크 | 실시간 가격 스트림에서 알림 조건 감지 + 쿨다운(1h) + 자동 비활성화. 조건 종류: `above` / `below` (목표가), `pct_change` (당일 변동률 %, 서버 평가), `drawdown` (고점 대비 낙폭 %, 서버 평가) |
...
| 배당 수집 | 평일 KST 18:00 보유종목 기준 국내(HHKDB669102C0) + 해외(HHDFS78330900) 배당 정보 KIS 조회 → dividends 테이블 upsert |
...
/dashboard                  → 메인 대시보드
  ...
  ├── /design-preview       → 디자인 시스템 미리보기 (프로덕션 차단; ALLOW_DESIGN_PREVIEW=1 시 E2E에서만 허용)
...
총 98개 엔드포인트 (모두 `/api/v1` prefix, 내부 API 별도):
...
| GET | `/dashboard/cash-summary` | 모든 KIS 계좌 예수금/평가금액 합산 (Pending BUY 차감, 계좌별 30초 캐시) |
