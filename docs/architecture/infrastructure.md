...
### 7.1 Same-origin API proxy (`frontend/next.config.ts`)

브라우저는 `https://joonwon.dev/api/*` (same-origin) 으로만 호출하고,
Next.js standalone 서버가 server-side rewrite 로 백엔드 컨테이너 (`http://backend:8000`) 에 forwarding 한다.
...
`API_PROXY_TARGET` 은 frontend 컨테이너의 server-side env (`docker-compose.yml`
에서 `http://backend:8000` 로 주입). 브라우저 번들에는 노출되지 않는다.
`NEXT_PUBLIC_API_URL` 은 빈 값 — axios baseURL 이 `/api/v1` 같은
same-origin path 가 되도록.

**왜 same-origin 인가?** 직전에는 `https://api.joonwon.dev` 를 클라이언트에
하드코딩해 모든 요청이 cross-subdomain 으로 나갔다. iOS Safari ITP 가
`Domain=.joonwon.dev` 의 cross-subdomain 인증 cookie 를 third-party
storage 로 분류해 비교적 짧은 주기로 evict 하면서, PWA 사용자가 하루에도
여러 번 강제 로그아웃 (`?reauth=1`) 되는 증상이 있었다. Same-origin 으로
좁히면 cookie 가 first-party 로만 동작해 ITP 의 cross-domain tracking
heuristic 에서 빠진다.

### 7.2 CORS 설정 (`app/main.py`)

```python
CORSMiddleware(
    allow_origins   = CORS_ORIGINS.split(","),   # e.g. "http://localhost:3000,https://joonwon.dev"
...
- **`If-None-Match` 허용**: ETag 캐싱 엔드포인트(`/chart/*`, `/analytics/*`)에서 브라우저가 자동 동봉하는 헤더. 누락 시 preflight 거절 → 본 요청이 "No ACAO" 로 차단되는 CORS 에러가 발생한다.
- **`expose_headers`**: CORS spec 상 simple response 헤더가 아닌 것은 명시되어야 브라우저 JS(`response.headers.get('etag')`)에서 읽을 수 있다.
