...
- [x] 포트폴리오 목록 카드 UI
- [x] 포트폴리오 생성 모달 (이름, 통화 입력)
- [x] 포트폴리오 삭제 버튼

### 종목 추가/관리 UI
- [x] 포트폴리오 상세 페이지 (`/dashboard/portfolios/[id]`)
- [x] 종목 검색 다이얼로그 연결 (StockSearchDialog 활용)
- [x] 종목 추가 폼 (수량, 평균단가 입력)
- [x] 종목 수정 인라인 편집 or 모달
- [x] 종목 삭제 버튼 + 확인 다이얼로그

### 대시보드 페이지 (`/dashboard`)
- [x] 포트폴리오가 없을 때 빈 상태(empty state) UI
- [x] 보유 종목이 없을 때 빈 상태 UI + 종목 추가 유도 버튼
- [x] 대시보드 데이터 주기적 자동 새로고침 (30초 interval)

### 계좌 동기화 UI
- [x] KIS 자격증명 설정 페이지 (`/dashboard/settings`)
- [x] 계좌 수동 동기화 버튼 + 결과 토스트 알림
- [x] 동기화 이력 목록
