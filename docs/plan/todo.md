...
- [x] KIS price cache TTL 5분으로 단축
- [x] 대시보드 refresh 시 Redis 캐시 강제 무효화

### UX 고도화
- [x] Cmd+K / Ctrl+K 키보드 단축키로 종목 검색 열기
- [x] 비밀번호 변경 기능 + 기존 refresh token 전체 revoke
- [ ] KIS 자격증명 등록 시 API 연결 테스트 (저장 전 유효성 검증)
- [x] 포트폴리오 상세 — 거래 기록에 날짜 선택 (date picker)
- [x] 테이블 접근성 — sortable header에 role="button" + keyboard event
