...
- [x] PWA Web Push 알림 (모바일 브라우저 지원) — Sprint 17 Phase 5 에서 완료
...
| 15 | ~~PWA Web Push 알림~~ | ~~19-1~~ | Sprint 17 Phase 5 완료 | -- |
...
## Milestone R · Phase 3 Hybrid Redesign (브랜치: `feat/design-overhaul`, 2026-04-22)

10단계(R1~R10) 전부 완료. `docs/plan/tasks.md` 의 **Phase 3 Hybrid Redesign Follow-ups** 섹션에 후속 TASK-RD-N 9건.

### 주요 산출물 (머지 시 main 에 반영)
- [x] R1 · shadcn 프리미티브 10개 (+ /dashboard/design-preview)
- [x] R2 · DB 마이그레이션 `m5n6o7p8q9r0` — target_allocation / strategy_tag / dividends / routine_logs / alert 조건 확장
- [x] R3 · 7개 신규 API (/tasks/today, /dividends/upcoming, /stream, /analytics/benchmark-delta, /portfolios/:id/target-allocation + /rebalance-suggestion, PATCH /users/me)
- [x] R4 · `/dashboard` 홈 재구성 (hero + 목표링 + 벤치 + 예수금 + 오늘 할 것 + 섹터 + 배당)
- [x] R5 · `/dashboard/stream` 신규 탭
- [x] R6 · `/dashboard/stocks` 랜딩 + stock detail RangeIndicator + 모바일 sticky 매수/매도
- [x] R7 · Portfolio 상세 Tabs + 개요 탭 SectorBar
- [x] R8 · `/dashboard/rebalance` 신규 — 슬라이더 편집 + 제안 주문
- [x] R9 · `/onboarding` 5-step wizard
- [x] R10 · 모바일 tab bar safe-area + primary 토큰 정리

### 알려진 한계 (위 TASK-RD-1 ~ RD-9 참조)
- 배당 데이터는 스키마만 확보. 실제 KIS 수집 배치 미구현 → Stream/Home 에 `0건` 표시.
- `/analytics/benchmark-delta` 의 `mine_pct` 는 0 — 벤치 대비 차이 표기가 `-benchmark_pct` 로 나옴.
- `/dashboard/design-preview` 가 프로덕션 빌드에도 포함 (auth-gated).
- Stock detail 의 모바일 매수/매도는 `window.dispatchEvent` 만. OrderDialog 연결 필요.
- `/onboarding` 은 라우트만 생성. `/register` 성공 시 리다이렉트 미적용.
- 홈 hero 스파크라인은 dummy 데이터.
- Portfolio 상세의 "분석" Tab 이 아직 없음.
- Alert `pct_change` / `drawdown` 조건은 컬럼만 확장, 서버 평가 로직 미구현.
