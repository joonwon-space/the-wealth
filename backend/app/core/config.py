...
class Settings(BaseSettings):
    ...
    def reject_placeholder_secrets(self) -> "Settings":
        if self.JWT_SECRET_KEY in _PLACEHOLDER_SECRETS:
            raise ValueError("JWT_SECRET_KEY must be set to a strong random value (not the default placeholder)")
        if self.ENCRYPTION_MASTER_KEY in _PLACEHOLDER_SECRETS:
            raise ValueError("ENCRYPTION_MASTER_KEY must be set to a 64-char hex string (not the default placeholder)")
