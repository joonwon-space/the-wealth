...
def upgrade() -> None:
    op.add_column(
        "portfolios",
        sa.Column("kis_account_id", sa.Integer(), sa.ForeignKey("kis_accounts.id", ondelete="SET NULL"), nullable=True),
    )
    op.create_unique_constraint("uq_portfolios_kis_account_id", "portfolios", ["kis_account_id"])
