...
const INITIAL_NEW_ACCT: NewAcctForm = {
  label: "",
  account_no: "",
  acnt_prdt_cd: "01",
  app_key: "",
...
export function KisCredentialsSection(): React.ReactElement {
  ...
  const [newAcct, setNewAcct] = useState<NewAcctForm>({ ...INITIAL_NEW_ACCT });
  ...
  const handleAddAccount = async () => {
    ...
    try {
      ...
      try {
        ...
        toast.warning(
          "계좌는 등록됐지만 초기 동기화에 실패했습니다. 설정 > 실계좌 조회 & 동기화를 다시 시도해주세요."
        );
      }
    } catch {
  ...
  const handleSaveLabel = async (id: number) => {
    ...
    try {
      ...
      setKisAccounts((prev) =>
        prev.map((a) => (a.id === id ? { ...a, label: editLabel } : a))
      );
    ...
    <>
          ...
          {showAddAccount && (
            <div className="space-y-2 rounded-lg border p-3">
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                <div className="space-y-1">
                  ...
                  <Input
                    value={newAcct.label}
                    onChange={(e) =>
                      setNewAcct((f) => ({ ...f, label: e.target.value }))
                    }
                    placeholder="연금저축"
                    className="h-8"
                  />
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={newAcct.acnt_prdt_cd}
                    onChange={(e) =>
                      setNewAcct((f) => ({
                        ...f,
                        acnt_prdt_cd: e.target.value,
                      }))
                    }
                    placeholder="01"
                    className="h-8 font-mono w-16"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-muted-foreground">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setNewAcct((f) => ({
                      ...f,
                      is_paper_trading: e.target.checked,
                    }))
                  }
              ...
              <div className="space-y-1">
                ...
                <Input
                  value={newAcct.app_key}
                  onChange={(e) =>
                    setNewAcct((f) => ({ ...f, app_key: e.target.value }))
                  }
                  placeholder="PS7yzJ..."
                  className="h-8 font-mono"
                />
              ...
              <div className="space-y-1">
                ...
                <Input
                  type="password"
                  value={newAcct.app_secret}
                  onChange={(e) =>
                    setNewAcct((f) => ({ ...f, app_secret: e.target.value }))
                  }
                  placeholder="••••••••"
                  className="h-8"
                />
      ...
      <Card>
        <CardContent className="space-y-4 p-6">
          ...
          <p className="text-sm text-muted-foreground">
            등록된 KIS 계좌의 보유 종목을 조회하고 포트폴리오에 자동
            동기화합니다.
          </p>
          <Button
            ...
            balanceAccounts.map((acct) => (
              <div key={acct.account_no} className="space-y-3">
                ...
                {acct.error ? (
                  ...
                  <>
                    {acct.synced &&
                      (acct.synced.inserted > 0 ||
                        acct.synced.updated > 0 ||
                        acct.synced.deleted > 0) && (
                        <p className="text-xs text-green-600">
                          동기화: +{acct.synced.inserted} ~{acct.synced.updated}{" "}
                          -{acct.synced.deleted}
                        </p>
                      )}
                    {acct.usd_krw_rate && (
                      <p className="text-xs text-muted-foreground">
                        ...
                        <span className="tabular-nums font-medium">
                          {formatKRW(acct.usd_krw_rate)}/USD
                        </span>
                      </p>
                    )}
