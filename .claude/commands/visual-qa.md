...
> **Read-only 모드 (필수)** — 이 명령은 어떤 데이터도 mutate하지 않는다. 라이브 환경에서 사용자 본인 계정의 데이터를 보호한다.
>
> **금지 동작:**
> - 회원가입, 로그인 외 인증 변경 (비밀번호 변경 / 회원 탈퇴 / KIS 계정 등록·수정·삭제)
> - 포트폴리오 / 보유 종목 / 거래 / 주문 / 알림 / 워치리스트 생성·수정·삭제
> - 어떤 form submit / "저장" / "추가" / "삭제" / "주문" / "확인" 버튼 클릭
>
> **허용 동작:**
> - GET 요청, navigation, 스크린샷, snapshot, evaluate
> - 검색 다이얼로그 / 모달 / 다이얼로그 열기·닫기 (제출 금지)
> - 로그인 / 로그아웃, 다크모드 토글, 탭/필터 전환 (UI state만)
>
> **버튼 클릭 전 안전 체크:** 버튼 텍스트나 aria-label에 다음 키워드가 있으면 클릭하지 않는다 — `추가`, `생성`, `만들기`, `저장`, `등록`, `삭제`, `제출`, `매수`, `매도`, `주문`, `송출`, `체결`, `회원가입`, `탈퇴`, `해지`. 의심스러우면 클릭하지 않고 사용자에게 보고.
>
> 위반 시 즉시 중단하고 사용자에게 어떤 데이터가 영향받았는지 보고한다.
